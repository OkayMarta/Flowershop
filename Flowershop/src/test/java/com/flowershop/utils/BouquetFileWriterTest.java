...
class BouquetFileWriterTest {
    ...
    @Test
    void testSaveBouquet() throws IOException {
        ...
        assertTrue(lines.get(1).startsWith("Троянда,червоний,5,70.0,6.0,100.0,Німеччина,true,Фрідом"));
        assertTrue(lines.get(2).startsWith("Лілія,білий,3,25.0,16.0,80.0,Польща,2,сильний"));
        assertTrue(lines.get(3).startsWith("Орхідея,білий,1,30.0,7.0,150.0,Україна,Каттлея,червоний"));
        assertTrue(lines.get(4).startsWith("Гортензія,синій,4,80.0,30.0,500.0,Австрія,кругле,голубий"));

        // Перевірка назви аксесуару
        assertTrue(lines.get(7).startsWith("Обгортка,папір,білий,55.0,розмір: 60 x 48"));
        assertTrue(lines.get(8).startsWith("Кошик,дерево,коричневий,186.0,розмір: 53 x 32"));
        assertTrue(lines.get(9).startsWith("Листівка,папір,бежевий,45.0,текст привітання: З днем народженням!"));
        assertTrue(lines.get(10).startsWith("Стрічка,поліестр,червоний,10.0,ширина: 3.0 см, довжина: 33.0 см"));
